using RequeraSapConnector.Structure;
using System;
...
{
    public abstract class SapConnection : IDisposable
    {
        ...
        public DataTable ReadTable(string tableName, string[] fields = null, string[] where = null, int skip = 0, int count = 0)
        {
            fields = fields ?? new string[0];
            where = where ?? new string[0];
            ...
            //    result.GetTable<RfcDbField>("FIELDS")
            //);
            return this.GetStructureMapper().FromRfcReadTableToList(
                result.GetTable<Tab512>("DATA"),
                result.GetTable<RfcDbField>("FIELDS")
            );
        }
